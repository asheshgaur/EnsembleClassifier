...
void check_ret(int *mem, int *acc, string bn)
{
    ...
    *acc = 0;
    if (fp) {
        if (fscanf(fp, "mem = %d, acc = %d", mem, acc) != 2) {
            *mem = 0;
            *acc = 0;
        }
        pclose(fp);
...
int main(int argc, char *argv[])
{
    ...
    string single_rules = "__codex_single.rules";
    ...
    switch(split_or_not(mi, btdv, THRESHOLD)) 
    {
        case DEFINITELY_NOT:
            ...
            dump_rules(p_classifier, single_rules);
            //remove_redund(p_classifier, rb);
            run_building_program(cmd, H1, "test_building", single_rules);
