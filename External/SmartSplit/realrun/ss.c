...
int main(int argc, char *argv[])
{
    ...
    trace_cnt = i;
    long total_time = 0;
    uint32_t *predictions = (uint32_t *)malloc(trace_cnt * sizeof(uint32_t));
    ...
    for(i = 0; i < trace_cnt; i++) {
        if (predictions[i] == (uint32_t)g_linear_search(ft + i * MAXDIMENSIONS)) {
            correct++;
        }
    }
    size_t incorrect = trace_cnt - correct;
    double accuracy = trace_cnt ? (100.0 * correct / trace_cnt) : 0.0;
#ifdef STAT
    ...
    printf("gbps: %.1fGbps\n", gbps);  
    printf("accuracy: %.6f%% (%zu/%d)\n", accuracy, correct, trace_cnt);
    printf("BENCHMARK.MEMORY_BYTES=%u\n", total_size);
    printf("BENCHMARK.TIME_NS_TOTAL=%ld\n", total_time);
    printf("BENCHMARK.TIME_NS_AVG=%.6f\n", ave_time);
    printf("BENCHMARK.ACCURACY_PERCENT=%.6f\n", accuracy);
    printf("BENCHMARK.CORRECT=%zu\n", correct);
    printf("BENCHMARK.INCORRECT=%zu\n", incorrect);
    printf("BENCHMARK.PACKETS=%d\n", trace_cnt);
